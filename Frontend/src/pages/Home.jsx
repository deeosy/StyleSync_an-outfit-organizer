...
function Home() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-6">Wardrobe Dashboard</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h2 className="text-lg font-semibold mb-4">Wardrobe Stats</h2>
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Total Items: <span className="font-medium text-gray-900">{wardrobeItems.length}</span></p>
            <p className="text-sm text-gray-600">Saved Outfits: <span className="font-medium text-gray-900">{savedOutfits.length}</span></p>
            {Object.entries(categoryCounts).map(([category, count]) => (
              <p key={category} className="text-sm text-gray-600">
                {category.charAt(0).toUpperCase() + category.slice(1)}: <span className="font-medium text-gray-900">{count}</span>
              </p>
            ))}
          </div>
        </div>
          ...
          {unusedItems.length > 0 ? (
            <ul className="space-y-2">
              ...
              {unusedItems.length > 5 && (
                <li className="text-sm text-gray-600">And {unusedItems.length - 5} more...</li>
              )}
            </ul>
          ) : (
            <p className="text-sm text-gray-600">Great job! You've worn all your items.</p>
          )}
        </div>
